...
from pymoysklad.json.exceptions import ApiError, ERRORS
...
class Requester:
    ...
    def _check_for_errors(func):
        ...
        def wrap(self, *args, **kwargs):
            ...
            if isinstance(answer, list):
                if any("errors" in x for x in answer):
    ...
    @_check_for_errors
    def get(self, url: str, params: dict | None = None, raw=False):
        self.session.auth = self._auth
        answer = self.session.get(
            urljoin(ENDPOINT, url),
            params=params,
    ...
    def __init__(self, auth: str | tuple[str, str]):
        ...
        self.session = LimiterSession(
            auth=self._auth, limiter=Limiter(RequestRate(45, Duration.SECOND * 3))
        )
